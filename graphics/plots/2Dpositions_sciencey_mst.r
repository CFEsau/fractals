...
args <- commandArgs()
#print(args)
f <- as.numeric(args[6]); q <- as.numeric(args[7]); k <- as.integer(args[8])

library(animation)
library(Hmisc) #for minor tick marks
source(paste0("/local/cfe/backed_up_on_astro3/Github/",
        ...
        #saveVideo({
          ...
          for (i in 1:nsnaps){
            ...
            if (plotmst) {
              ...
              mst_infn <- paste0('../cluster_', clustype, '/lambda/coords/',
                                 infn, '_objconnections_', proj[p], '.dat')
