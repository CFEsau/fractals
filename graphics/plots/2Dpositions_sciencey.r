...
#for (k in 1:nkvals){
  ...
  #axlim <- 1.0
  #fovlim <- 1.0 #Field of view limit in p
  ...
  for (i in 1:nsnaps){
    ...
    star_df <- data.frame(rx_all,ry_all,rz_all,mstar_all)[order(-mstar_all),]
      ...
      outfn <- sprintf('%02ssnap%04d_fdim.png',proj,i) # output file name
      #set up output plot
      ...
      #Add stars:
      plot(xdat, ydat, cex=0.5,pch=20,col='gray60',
           #axes = FALSE,
	         #ann = FALSE,
      ...
      #} #lwd for line width. Default lwd=1
