...
for i in range(1,nfile+1):
    ...
    ax1.scatter(x, y, z, marker='.',s=2)
    plt.xlabel('X-Position (pc)')
    plt.ylabel('Y-Position (pc)')
    plt.zlabel('Z-Position (pc)')
    plt.title('Simulation')
    plt.grid(True)
    plt.axis([-5, 5, -5, 5, -5, 5])
    plt.savefig(ifname+'.xyz.png', dpi=my_dpi, bbox_inches='tight')

os.system('avconv -y -r 15 -i ' + sim + '/snapshots/snap%04d.xyz.png -s 1024x800 ' + sim + '/xyz.mp4')
