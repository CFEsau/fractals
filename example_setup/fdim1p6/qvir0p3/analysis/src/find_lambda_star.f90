...
SUBROUTINE find_lambda_star(snapshoti,ni)
...
! Find the median length in the tree, and find the length of a tree made from these
  obj_mst = (nmst-1) * length(length_list( NINT(REAL(nmst-1)/2.) ) )
...
! Find the median length in the tree, and find the length of a tree made from these
     rand_mst(j) = (nmst-1) * length(length_list( NINT(REAL(nmst-1)/2.) ) )
...
!Sort random MST lengths:
  ...
  lambda_star(snapshoti) = avranmst/obj_mst
  l_low_star(snapshoti) = ranlow/obj_mst
  l_up_star(snapshoti) = ranup/obj_mst
...
END SUBROUTINE find_lambda_star
