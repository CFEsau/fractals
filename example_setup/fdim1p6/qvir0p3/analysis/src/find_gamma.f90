...
!
!This is the GEOMETRIC version, taken from Olczak, 2011, A&A, 532, A119
!
!This version also uses the 1/6 and 5/6 uncertainties (from a cumlative distribution 
!of the random MSTs). This is different to the standard deviation used in Olczak (2011),
!but the assumption that the random MSTs are guassian is potentially dodgy.
!Especially if one is analysing clusters with binaries.

SUBROUTINE find_gamma(snapshoti,ni)
...
!Split obj_r across x,y,z arrays. Just helps readability in mst subroutine.
  ...
  DO i = 1,nmst-1
     obj_mst = obj_mst + LOG(length(i))  !Add the edges of the mst to find 
  END DO                      !the total length

!Calculate the geometric mean
   obj_mst = EXP( (1./REAL(nmst-1)) * obj_mst)
...
6       CALL RANDOM_NUMBER(rand)
     ...
     DO i = 1,nmst - 1
        rand_mst(j) = rand_mst(j) + LOG(length(i))    !Add the edges of the mst
     END DO                                  !to find the total length

      rand_mst(j) = EXP( (1./REAL(nmst-1)) * rand_mst(j) )
  END DO
...
!Sort random MST lengths:
  ...
  gamm(snapshoti) = avranmst/obj_mst
  g_low(snapshoti) = ranlow/obj_mst
  g_up(snapshoti) = ranup/obj_mst
