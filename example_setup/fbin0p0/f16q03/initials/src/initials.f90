...
! Open input file
  ...
  READ(1,*) nsim           ! number of simulations
...
! kdum equal to simulation number:
     kdum=k
! kdum set manually (remember to change nsim to 1!):
     !kdum=1
! Give seed a pseudorandom value based on k between (100k-100) and (100k)
! This makes sure all seeds will be different as they are between different
! equally-sized ranges.
     !kdum=k
     !random = (ran2(kdum)*(100))+((100*k)-99)
     !kdum=INT(random)
...
! Separation needs to be a Real and in parsecs.
           asep=asep/pc
           rsep=REAL(asep)
           CALL assign_binary(m(nstars+1),m(nstars+2),ecc,rsep, &
                &              v(1:3,nstars+1),v(1:3,nstars+2), &
                &               r(1:3,nstars+1),r(1:3,nstars+2),kdum)
