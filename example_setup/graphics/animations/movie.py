...
ncores = int(0)
#proj=str('xy')
proj = raw_input("Enter projection: ")

def generate_snapshot(ifname,snapname,proj,qvir,fdim,kval,xy_box,z_box,fcom_elements):
        import matplotlib.pyplot as plt
#	from mpl_toolkits.mplot3d import Axes3D
	import numpy as np
	...
	if proj == "3D":
		...
		ax1.scatter(x_cluster[np.where(m_cluster>=mass_selected[0])],
                            z_cluster[np.where(m_cluster>=mass_selected[0])],
                            y_cluster[np.where(m_cluster>=mass_selected[0])], marker='*',color='red', s=20)
		...
		ax1.scatter((x_cluster[np.where(m_cluster>=mass_selected[0])]-fcom_element_x),
                            (y_cluster[np.where(m_cluster>=mass_selected[0])]-fcom_element_y),
		...
		ax1.scatter(y_cluster[np.where(m_cluster>=mass_selected[0])],
                            z_cluster[np.where(m_cluster>=mass_selected[0])],
                            marker='*',color='red', s=20)
        ...
        if proj != '3D':
	        ...
	        plt.grid(True)
                ...
                plt.text(0.70*xy_box,0.85*xy_box,"N_Stars = " +
                         str(len(x_cluster[np.where(m_cluster>=mass_selected[0])])))
                plt.text(-1.1*xy_box,-1.1*xy_box,"fbin = 0%",fontsize=11)
                plt.text(0.94*xy_box,-1.1*xy_box,kval,fontsize=10)
        
        
        plt.savefig(str(ifname)+'.'+proj+'_'+str(xy_box)+'pc.png', dpi=my_dpi, bbox_inches='tight')
        plt.close()
	del(mass_srt_by_m)
	del(mass_selected)
        return snapname
...
kval = raw_input("Enter kval: ")

sim = '../../fbin0p0/fdim1p6/qvir0p3/outputs/runinv_k'+str(kval)
...
fcom_file = open(sim+'/cluster_FoV5pc/c_of_m_xy.dat', 'r')
fcom_data=[]
...
for fcom_datum in fcom_data:
	fcom_elements.append(fcom_datum.replace(" ",",").replace(',,',',').replace(',,',',').split(',')[1:])
	#print(fcom_datum.replace(" ",",").replace(',,',',').replace(',,',',').split(',')[1:])
...
#	print fcom_elements
	if ncores==-1:
   		generate_snapshot(ifname[i-1],snapname[i-1],proj,qvir,fdim,kval,xy_box,z_box,fcom_elements[i])

if ncores!=-1:   	
	irange = np.arange(0, i-1, 1)
	jobs = [(ix, job_server.submit(generate_snapshot,args=(ifname[ix],snapname[ix],proj,qvir,fdim,kval,xy_box,z_box,fcom_elements[ix]),depfuncs=(), modules=("matplotlib","numpy","mpl_toolkits"), globals=globals())) for ix in irange]
        #(the "mpl_toolkits" is removed in another version of this code...
        # not sure why, haven't needed to check. If problem, comment out)
	for ix, job in jobs:
           px=job()
           del(px)
	   print snapname[ix]
...
os.system('avconv -y -r 24 -i ' + sim + '/snapshots/snap%04d.' + proj + '_' + str(xy_box) + 'pc.png -s 1024x800 ' + parentdir + '/movies/' + proj + '_' + str(kval) + '_' + str(xy_box) + 'pc.mp4')
