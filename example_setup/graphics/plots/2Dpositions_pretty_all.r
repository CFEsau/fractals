...
for (k in 1:nkvals){
  ...
  nmst <- 10
  axlim <- 1.0
  fovlim <- 1.0 #Field of view limit in pc
  ...
  for (i in 1:400){
    ...
    outfn <- sprintf('grid/2Dsnap%04d.png',i) # output file name
    ifelse(!dir.exists('grid'),
           dir.create('grid'), FALSE)
    #set up output plot
    ...
    for (p in 1:3){
      ...
      #time <- (i/nsnaps)*10
      #title(main = sprintf("Time = %.2f Myr",time),line=-1,adj=0.1,col.main = "gray80")#, cex.main = 0.9)
